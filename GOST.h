...
unsigned long uchar2ulong(unsigned char i[4]) {
  return (
           (((unsigned long)(i[0])) << 0) + (((unsigned long)(i[1])) << 8) +
...
uint16_t GOST_Encrypt_Block_ECB(
  unsigned char *plaintext, uint16_t currentSize, unsigned char *ciphertext,
  unsigned char *key, unsigned char *IV
) {
  // plaintext can be of any size
  // ciphertext's length has to be a multiple of 8
  GOST_CTX ctx;
  GOST_Init(&ctx, key, IV);
  uint16_t finalSize;
  uint8_t difference;
  difference = currentSize % 8;
  if (difference != 0) {
    finalSize = difference + currentSize;
  } else finalSize = currentSize;
  memcpy(ciphertext, plaintext, currentSize);
  memset(ciphertext + currentSize, difference, difference);
  for (int n = 0; n < finalSize; n += 8) {
    GOST_EncryptECB(&ctx, ciphertext + n);
  }
  return finalSize;
  // So that the user knows where to pick up the IV,
  // ie the last 8-byte block
}

uint16_t GOST_Decrypt_Block_ECB(
  unsigned char *ciphertext, uint16_t currentSize, unsigned char *plaintext,
  unsigned char *key, unsigned char *IV
) {
  // plaintext can be of any size
  // ciphertext's length has to be a multiple of 8
  GOST_CTX ctx;
  GOST_Init(&ctx, key, IV);
  memcpy(plaintext, ciphertext, currentSize);
  for (int n = 0; n < currentSize; n += 8) {
    GOST_DecryptECB(&ctx, plaintext + n);
  }
  return currentSize;
  // So that the user knows where to pick up the IV,
  // ie the last 8-byte block
